(function(){
    ...
    var cmdIndex = -1
    var SOCKET_URL = ''
    var MESSAGE_HISTORY = 100
    var COMMAND_HISTORY = 10
    ...
    function updateUserlist(){
        var $userList = $("#users")
        var userArray = Object.keys(userList).map(function(id){
                var user = userList[id]
                var $userEl = $("#users #user-" + id)
                if(!$userEl.length)
...
*/
        ...
        socket.on("taboo message", function(type, source, message){
            var source
            var messageHTML = "<li class='" + type + "'><span class='source'>{{source}}</span><span class='message'>" + message + "</span></li>"
            if(source === 'server'){
                source = ""
            }
